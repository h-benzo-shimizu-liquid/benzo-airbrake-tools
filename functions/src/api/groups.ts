...
export default async (req: express.Request, res: express.Response, next: express.NextFunction): Promise<express.Response | void> => {
	try {
		const requestBody: RequestGroups = req.body;
		const url: string = `https://api.airbrake.io/api/v4/projects/${requestBody.projectId}/groups?key=${requestBody.userKey}&limit=${requestBody.limit}&page=${requestBody.page}&order=notice_count`;
